<template>
  <div class="overflow-y-scroll scrollbar-y-hide vh-100">
    <h2 v-if="isNewArticle" class="py-4 py-md-5 fw-bold">新增文章</h2>
    <h2 v-else class="py-4 py-md-5 fw-bold">編輯文章</h2>
    ...
    <div>
      ...
      <div>
        ...
        <v-form v-slot="{ errors }" class="d-flex flex-column gap-3" @submit="updateArticleInfo">
          ...
          <div class="mb-5">
            <h5 class="d-inline-block me-2 mb-3 fw-bold">關聯商品</h5>
            <span class="text-info fs-info">最多請選擇 3 個商品</span>
            <div class="row g-3 g-md-1">
              <div class="col-12 col-md-6">
                <div class="input-group">
                  <label class="input-group-text">類別</label>
                  <select class="form-select" v-model="relatedProduct.currentCategory">
                    <option value="" disabled>選擇商品類別</option>
                    <option v-for="(item, idx) in relatedProduct.categoryList" :key="idx" :value="item">
                      {{ item }}
                    </option>
                  </select>
                </div>
          ...
          <div>
            <h5 class="mb-3 fw-bold">文章發布狀態</h5>
...
export default {
  ...
  watch: {
    // 關連產品分類 至 名稱
    'relatedProduct.currentCategory': function () {
      const tempProductList = Object.values(productsStore.allProducts)

      this.relatedProduct.productList = tempProductList.filter((item) => {
        const isEnabled = item.is_enabled
        const isRelatedProduct = item.category === this.relatedProduct.currentCategory
